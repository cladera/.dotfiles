	...
	vim.api.nvim_create_autocmd("LspAttach", {
		...
		callback = function(ev)
			...
			k.map(
				"n",
				"gi",
				k.cmd("lua vim.lsp.buf.implementation()"),
			...
			k.map(
				"n",
				"<leader>lj",
				k.cmd("lua vim.diagnostic.goto_next()"),
				{ desc = "Next Diagnostic", buffer = ev.buf }
			)
			k.map(
				"n",
				"<leader>lk",
				k.cmd("lua vim.diagnostic.goto_prev()"),
				{ desc = "Prev Diagnostic", buffer = ev.buf }
			)
			...
			if telescope_ok then
				k.map(
					"n",
					"<leader>ls",
					k.cmd("Telescope lsp_document_symbols"),
					{ desc = "Document Symbols", buffer = ev.buf }
				)
				k.map(
					"n",
					"<leader>lS",
					k.cmd("Telescope lsp_dynamic_workspace_symbols"),
					{ desc = "Workspace Symbols", buffer = ev.buf }
				)
				k.map(
					"n",
					"<leader>ld",
					k.cmd("Telescope diagnostics layout_strategy=bottom_pane previewer=false"),
